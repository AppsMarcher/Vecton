...
-- 047: gestao MATRICIAL no detalhe do Painel de Vendas.
-- Alem do roteamento vertical (Pecuaria de Sul/Norte -> coordenacao Pecuaria/Paulo),
-- cada territorio tem uma CASA geografica = a coordenacao da sua atribuicao de
-- Grao (SP/PR/RS -> Sul; GO_MG/BA/TO/... -> Norte; MT/MS/RO -> Oeste; EX -> Exp.).
--
-- O RPC passa a devolver, por linha:
--   * regiao  = casa geografica do territorio (coord do Grao). Usada no front pra
--               montar o detalhe geografico: ao filtrar Sul, aparecem TODOS os
--               territorios do Sul, com Grao E Pecuaria, mesmo os "so Pecuaria"
--               (cuja Pecuaria some no Paulo pelo rollup). E a visao matricial.
--   * orfao   = responsavel da linha == gestor da coordenacao de ROTEAMENTO dela.
--               O front usa isso pra NAO criar card do orfao (consolida no gestor),
--               valendo tanto no corte vertical (Paulo) quanto no geografico.
--
-- Totais/hero/cards do topo seguem VERTICAIS (por coordenacao de roteamento),
-- sem duplicar Pecuaria. O consolidado do detalhe tambem vem do roteamento (o
-- front usa 'coordenacao'); as tabelas por territorio vem da 'regiao'. Assim o
-- consolidado da aba Sul/Norte fica so-Grao e a Pecuaria aparece so informativa
-- nas tabelas de cada territorio.
--
-- Mantem o CTE 'slots' da 046 (cards "A definir" mesmo zerados).
--
-- O RETURNS TABLE ganhou colunas novas (regiao, orfao) -> nao da pra CREATE OR
-- REPLACE por cima; precisa DROP antes (Postgres nao troca o tipo de retorno).

drop function if exists public.comercial_painel_vendas(uuid, integer, integer, text, uuid);
...
begin
  ...
  select
    ...
    (co.gestor is not null
      and coalesce(fat.responsavel, cart.responsavel, meta.responsavel, y1.responsavel, y2.responsavel, y3.responsavel, sl.responsavel) is not null
      and lower(btrim(coalesce(fat.responsavel, cart.responsavel, meta.responsavel, y1.responsavel, y2.responsavel, y3.responsavel, sl.responsavel)))
