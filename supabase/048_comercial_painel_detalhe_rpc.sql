...
-- 048: RPC de DETALHE do Painel de Vendas (drill-down do popover).
-- Ao clicar no rotulo FATUR. (origem FAT -> NF) ou FAT.+CART. (FAT+CART -> NF+Ped)
-- de uma tabela, o front busca aqui as transacoes que formam aquele numero.
--
-- Escopo espelha o card clicado:
--   * card de TERRITORIO -> p_territorio + p_linhas (Grao/Pecuaria do card).
--   * card CONSOLIDADO   -> p_coordenacao + p_linhas (todos os territorios da
--                           coordenacao de roteamento; front pede coluna Territorio).
-- Janela = mesma do painel (mes/ytd/fy sobre p_year/p_month).
-- So maquinas (Grao/Pecuaria) por enquanto — Pecas/Transgrain/Acessorios fora.
