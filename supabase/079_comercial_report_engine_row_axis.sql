...
-- Generaliza o motor dos relatorios comerciais. Ate aqui, comercial_report_execute
-- (069, com os patches de nome/cod_vendedor aplicados em 071/072) so sabia montar
-- 1 linha por vendedor. Extrai o corpo pesado para comercial_report_compute,
-- parametrizado (sem depender de report_id/versao salva), para poder ser chamado
-- tanto pelo execute() quanto pelo preview() (migration 080). Adiciona o eixo de
-- linha (row_axis: seller|month|product|culture|territory), filtro por territorio
-- (territory_ids) e a metrica de margem em R$ (soma(valor*mb_pct), sinal
-- preservado nas devolucoes porque sum() em SQL ja ignora mb_pct nulo).
--
-- row_axis ausente no config == 'seller': o ramo 'seller' abaixo reproduz
-- exatamente a logica de 069+071+072 (incluindo o wrap de nome atual via
-- comercial_apply_current_seller_names) — Bateu-Levou e Final de Ano nunca
-- gravaram row_axis, entao continuam idênticos.
...
begin
  ...
  v_uses_target := v_primary in ('quantity', 'revenue') and (v_requires_target or v_evaluation in (
  ...
  -- Eixo Mes: 1 linha por mes do periodo, sem ranking/elegibilidade/premiacao.
  -- Meta so existe pra quantidade/faturamento (planejado nao tem margem).
    ...
    select
      ...
      jsonb_build_array(
        jsonb_build_object('key', 'periods', 'label', 'Meses', 'value', count(*)),
...
-- Casca fina: resolve report+versao, reaproveita snapshot oficial quando
-- encerrado, chama comercial_report_compute e oficializa quando solicitado.
...
begin
  ...
  v_payload := public.comercial_report_compute(
    ...
    v_report.status, v_report.modalidade, v_report.data_inicio, v_report.data_fim,
