...
-- Acrescenta a meta (Budget) mensal de Peças à evolução mensal, seguindo o
-- mesmo caminho já usado e validado pelo comercial_painel_tipos: produto ->
-- comercial_tipos.nome = 'Peças' (não linha_negocio_id, que tem histórico
-- de inconsistência nessa tabela — ver supabase/_diag_meta_pecas_jenifer.sql).
-- Meta de Peças é nacional e única (decisão de produto já registrada na
-- 087_comercial_painel_pecas_por_vendedor.sql): por isso ela NÃO respeita os
-- filtros de vendedor/UF/município/cliente/SKU do relatório, só o ano/mês.
...
begin
  ...
  meta as materialized (
    select le.reference_month, sum(le.valor) meta
    from public.comercial_planejado_ledger_entries le
    join public.comercial_produtos pr on pr.id = le.produto_id
    join public.comercial_tipos t on t.id = pr.tipo_id
    where le.organization_id = p_org and t.nome = 'Peças'
      and le.scenario_id is null
      and le.reference_year = p_year and le.reference_month between 1 and p_month
    group by le.reference_month
  ),
  series as (
  ...
  select jsonb_build_object(
    ...
    'months', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'meta', coalesce((select mt.meta from meta mt where mt.reference_month = s.m), 0)
