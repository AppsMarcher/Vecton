BEGIN;
CREATE TABLE public.fc_scenarios (
...
CREATE INDEX fc_scenarios_owner_year ON public.fc_scenarios(organization_id,created_by,reference_year);
ALTER TABLE public.fc_scenarios ENABLE ROW LEVEL SECURITY;
GRANT SELECT ON public.fc_scenarios TO authenticated;
CREATE POLICY "Own FC scenarios" ON public.fc_scenarios FOR SELECT TO authenticated
 USING(created_by=auth.uid() AND public.can_read_fc(organization_id));
CREATE FUNCTION public.save_fc_scenario(target_org uuid, payload jsonb)
...
BEGIN
 ...
 IF payload->>'source_scenario_id' IS NOT NULL THEN
  SELECT * INTO source FROM public.fc_scenarios WHERE id=(payload->>'source_scenario_id')::uuid AND organization_id=target_org AND created_by=auth.uid();
 ...
 INSERT INTO public.fc_scenarios(organization_id,created_by,name,reference_year,base_file_name,report_data)
 VALUES(target_org,auth.uid(),btrim(payload->>'name'),(base->>'year')::integer,filename,base) RETURNING id INTO result;
 RETURN result;
END; $$;
REVOKE ALL ON FUNCTION public.save_fc_scenario(uuid,jsonb) FROM public;
GRANT EXECUTE ON FUNCTION public.save_fc_scenario(uuid,jsonb) TO authenticated;
COMMIT;
