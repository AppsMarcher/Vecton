...
-- Corrige "Dias sem comprar" no relatorio Performance Geografica de Pecas
-- (card Top Clientes). O calculo usava sempre o ULTIMO DIA do periodo
-- filtrado (ex.: 31/12 quando o filtro e "Ano") como data de referencia,
-- em vez da data atual. Com filtro fy/ytd apontando para um periodo que
-- ainda nao terminou, isso contava dias futuros que ainda nao aconteceram
-- (ex.: cliente com ultima compra em 03/09/2026 aparecia com 119 dias sem
-- comprar em 11/09/2026, pois o calculo ia ate 31/12/2026).
-- Fix: usa least(fim do periodo filtrado, data de hoje) como referencia.
...
begin
  ...
  -- "Dias sem comprar" nunca deve contar dias que ainda nao aconteceram:
  -- usa o fim do periodo filtrado, mas nunca alem de hoje.
  v_period_end := least((v_ref + interval '1 month - 1 day')::date, current_date);
...
comment on function public.comercial_pecas_geo_performance(uuid,integer,integer,text,text,text,text,uuid,text,text)
  is 'Performance geografica de Pecas nas visoes nf, cart ou nf_cart. Dias sem comprar usa least(fim do periodo, hoje) como referencia.';
