...
-- Transgrain/Acessórios (linha "Outros") na carga de vendas realizadas:
--
-- Decisão do usuário: eles ENTRAM na carga/banco, com o CARIMBO do território
-- (território OBRIGATÓRIO na validação, igual Grão/Pecuária), mas SEM regra de
-- roteamento por enquanto -> coordenação/responsável ficam NULOS (a definir).
-- A linha é VÁLIDA (não bloqueia o lote). O dado fica guardado por regional pra
-- quando o modelo do relatório/painel for definido (outro escopo).
--
-- Máquinas (Grão/Pecuária por regional) e Peças (nacional/Jenifer) inalterados.

-- Coordenação passa a aceitar nulo no ledger (linhas "Outros" sem roteamento).
alter table public.comercial_realizado_ledger_entries
  alter column coordenacao_id drop not null;

-- Trigger de validação/derivação: mesma da 039, com o tratamento de "Outros".
...
begin
  ...
  -- Obrigatorio para tudo, MENOS Pecas (nacional). Transgrain/Acessorios (Outros)
  -- exigem territorio (o carimbo da regional onde foram vendidos).
  ...
  --   Outros        -> FORA de escopo de roteamento por enquanto: guarda so o
  --                    carimbo do territorio (obrigatorio, ja resolvido acima);
  --                    coordenacao/responsavel ficam NULOS (a definir). Nao e erro.
