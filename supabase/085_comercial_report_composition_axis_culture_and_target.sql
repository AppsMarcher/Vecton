...
-- Duas correcoes no relatorio "Composicao por produto/cultura" (row_axis in
-- ('product','culture')), encontradas pelo usuario ao gerar o relatorio pela
-- primeira vez:
--
-- 1) Nao tinha Meta/Var: a branch product/culture nunca calculava meta (o
--    comentario antigo dizia "planejado nao tem grao fino o suficiente pra
--    comparar aqui", mas o planejado tem produto_id (por tipo/cultura) igual
--    ao realizado, entao da pra agregar meta por tipo/cultura do mesmo jeito
--    que ja se faz por vendedor (branch seller) e por mes (branch month).
--    Sem isso, v_uses_target tambem ficava sempre falso pra esse eixo (so
--    ligava com v_row_axis='month', igual o fix da 081) e nenhuma coluna de
--    Meta/Atingimento aparecia mesmo com meta cadastrada.
-- 2) "Separar por cultura" nao fazia nada: o checkbox de separar por cultura
--    so existe no formulario avancado e so e lido pela branch 'seller'
--    (v_group_culture). A branch product/culture agrupava OU por tipo OU por
--    cultura (nunca os dois), entao o template "Composicao por produto/
--    cultura" (row_axis='product' fixo) nunca separava Graos de Pecuaria.
--    Fix: quando v_row_axis='product', agrupa por (tipo, cultura) sempre —
--    cada linha carrega 'segment'=cultura (Graos/Pecuaria/Sem cultura), que o
--    front usa pra montar 3 secoes (igual ja faz no Bateu-Levou por
--    segmento). Pecas/Transgrain/Acessorios (sem cultura) caem na secao
--    "Sem cultura". Quando v_row_axis='culture', mantem 1 linha por cultura
--    (sem sub-quebra), so ganhando meta.
--
-- Reescreve a funcao inteira (nao um patch textual) porque a mudanca
-- estrutura novas CTEs (target/keys/combined) na branch product/culture — as
-- branches seller/month/territorio permanecem identicas ao estado atual
-- (079+081+082+083+084), so incorporadas aqui pra manter 1 unica definicao
-- coerente da funcao.
...
begin
  ...
  v_uses_target := v_primary in ('quantity', 'revenue') and (v_row_axis in ('month', 'product', 'culture') or v_group_culture or v_requires_target or v_evaluation in (
  ...
  -- Eixos Produto/Cultura: quando v_row_axis='product', 1 linha por
  -- (grupo de produto/tipo, cultura) — cultura vira 'segment' (Graos/
  -- Pecuaria/Sem cultura), usado pelo front pra montar 3 secoes (mesmo
  -- padrao do Bateu-Levou por segmento). Quando v_row_axis='culture', 1
  -- linha por cultura, sem sub-quebra (segment sempre null). Meta agregada
  -- do planejado por tipo/cultura (mesmo padrao de atribuicao vigente usado
  -- nas branches seller/month), corrigindo o "sem meta" da versao anterior.
    ...
    with filtered as (
      ...
      select
        ...
        case when v_row_axis = 'product' then culture_name else null end as segment,
        sum(quantidade) as quantity,
        sum(valor) as revenue,
        sum(valor * mb_pct) as margin
      from filtered
      group by
        case when v_row_axis = 'product' then product_group else culture_name end,
        case when v_row_axis = 'product' then culture_name else null end
    ), filtered_target as (
      select t.nome as product_group, coalesce(c.nome, 'Sem cultura') as culture_name,
             m.quantidade, m.valor
      from public.comercial_planejado_ledger_entries m
      join lateral (
        select a.* from public.comercial_atribuicao_responsavel a
        where a.organization_id = m.organization_id
      ...
      select
        case when v_row_axis = 'product' then product_group else culture_name end as label,
        case when v_row_axis = 'product' then culture_name else null end as segment,
        sum(quantidade) as target_quantity,
        sum(valor) as target_revenue
      from filtered_target
      group by
        case when v_row_axis = 'product' then product_group else culture_name end,
        case when v_row_axis = 'product' then culture_name else null end
    ), keys as (
      select label, segment from grouped
      union
      select label, segment from target_grouped
    ), combined as (
      select
        k.label, k.segment,
        coalesce(g.quantity, 0) as quantity,
        coalesce(g.revenue, 0) as revenue,
        coalesce(g.margin, 0) as margin,
        coalesce(tg.target_quantity, 0) as target_quantity,
        coalesce(tg.target_revenue, 0) as target_revenue
      from keys k
      left join grouped g on g.label = k.label and g.segment is not distinct from k.segment
      left join target_grouped tg on tg.label = k.label and tg.segment is not distinct from k.segment
    ), scored as (
      select c.*,
        case when v_primary = 'quantity' then c.quantity
             when v_primary = 'revenue' then c.revenue
             else c.margin end as realized,
        case when v_primary = 'quantity' then c.target_quantity
             when v_primary = 'revenue' then c.target_revenue
             else 0 end as target
      from combined c
    ...
    select
      coalesce(jsonb_agg(jsonb_build_object(
        'row_key', coalesce(f.segment, '') || '|' || f.label, 'label', f.label, 'segment', f.segment,
        'quantity', f.quantity, 'revenue', f.revenue, 'margin', f.margin,
        'target_quantity', f.target_quantity, 'target_revenue', f.target_revenue,
        'realized', f.realized, 'target', f.target, 'attainment_pct', f.attainment_pct
      ) order by f.segment nulls last, f.realized desc nulls last, f.label), '[]'::jsonb),
      jsonb_build_array(
        ...
        jsonb_build_object('key', 'target_total', 'label', 'Meta total', 'value', coalesce(sum(f.target), 0))
      ) else '[]'::jsonb end
    into v_rows, v_summary
    from final_rows f;
    ...
    if v_row_axis = 'product' then
      v_columns := v_columns || jsonb_build_array(jsonb_build_object(
        'key', 'segment', 'label', 'Cultura', 'type', 'text', 'visible', true, 'order', 2, 'primary_metric', false));
    end if;
    v_columns := v_columns || jsonb_build_array(
      jsonb_build_object('key', 'realized', 'label',
        case when v_primary = 'quantity' then 'Volume' when v_primary = 'revenue' then 'Faturamento' else 'Margem' end,
