  ...
  function createComercialReportsModule(deps) {
    ...
    function renderPayload(container, payload, scenarios, scenarioId) {
      ...
      container.innerHTML = `<div class="vcr-report">
        ...
        <div class="vcr-table-wrap"><table class="vcr-table"><thead><tr>${columns.map((column) => `<th class="${["currency", "percentage", "number", "integer"].includes(column.type) ? "num" : ""}">${escapeHtml(column.label)}</th>`).join("")}</tr></thead><tbody>${rows.length ? rows.map((row) => `<tr data-vcr-code="${escapeHtml(row.cod_vendedor || "")}">${columns.map((column) => { const value = row[column.key]; const numeric = ["currency", "percentage", "number", "integer"].includes(column.type); const pill = ["boolean", "status"].includes(column.type); return `<td class="${numeric ? "num" : ""}">${pill ? `<span class="vcr-pill ${value === true || value === "Vencedor" || value === "Atingiu" ? "ok" : value === false || value === "Inelegível" || value === "Não atingiu" ? "no" : ""}">${escapeHtml(formatValue(value, column))}</span>` : escapeHtml(formatValue(value, column))}</td>`; }).join("")}</tr>`).join("") : `<tr><td colspan="${columns.length}" class="vcr-empty">Sem resultados para o período.</td></tr>`}</tbody></table></div>
