(function attachVectonComercialReports(window) {
  ...
  function createComercialReportsModule(deps) {
    ...
    function ensureStyles() {
      ...
      style.id = "vcr-style";
      style.textContent = `
        .vcr-create-card{border-style:dashed!important;align-items:center!important;justify-content:center;gap:6px;text-align:center;color:var(--theme-ink-teal, #14b8a6)!important;font-weight:600}.vcr-create-card span:last-child{font-size:11.5px;line-height:1.25}
        .vcr-card-status{font-size:9px;text-transform:uppercase;letter-spacing:.06em;color:var(--text-faint)}
        .vcr-edit-btn{position:absolute;bottom:10px;right:10px;width:26px;height:26px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;color:var(--text-faint);background:var(--theme-surface-tint, rgba(255,255,255,.05));border:1px solid var(--theme-border-subtle, rgba(255,255,255,.08));cursor:pointer;z-index:2}.vcr-edit-btn svg{width:13px;height:13px;pointer-events:none}.vcr-edit-btn:hover{color:var(--text);background:var(--theme-surface-tint, rgba(255,255,255,.12))}
        .vcr-overlay{position:fixed;inset:0;z-index:9900;background:var(--theme-overlay, rgba(0,0,0,.64));display:flex;align-items:center;justify-content:center;padding:24px}
        .vcr-modal{width:min(1080px,96vw);max-height:94vh;overflow:hidden;background:var(--panel);border:1px solid var(--line);border-radius:18px;display:flex;flex-direction:column;box-shadow:0 30px 90px var(--theme-shadow-color, rgba(0,0,0,.58))}
        ...
        .vcr-period-seg{display:flex;gap:2px;background:var(--panel-alt);border:1px solid var(--line);border-radius:8px;padding:2px}.vcr-period-seg button{border:none;background:transparent;color:var(--text-soft);font:inherit;font-size:12px;font-weight:500;padding:6px 12px;border-radius:6px;cursor:pointer}.vcr-period-seg button.active{background:#4f7cff;color:#fff}
    ...
    // Colunas do drilldown que cada campanha esconde. O par considerado/motivo
    ...
    async function openMovements(reportId, codVendedor, scenarioId, reportName, segment, reportKind) {
      ...
      try {
        ...
        ].filter(([key]) => !HIDDEN_MOVEMENT_COLUMNS_BY_KIND[reportKind]?.has(key));
